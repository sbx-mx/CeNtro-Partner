...
export function RankingPage() {
  ...
  const indicatorSummaries = useMemo<IndicatorSummary[]>(() => displayedIndicators.map(indicator => {
    const items = visibleStores
      .map(store => store.indicators.find(item => item.indicator === indicator.indicator))
      .filter((item): item is IndicatorValue => Boolean(item))
    ...
    const applicable = items.reduce((sum,item) => sum + item.applicable, 0)
    const summaryItem: IndicatorValue = {
      ...indicator,
      value:averageValue,
      displayValue:undefined,
      status:averageValue === null ? 'na' : 'blank',
      score:null,
      fulfilled,
      applicable,
    }
    return {
      ...
      value:averageValue === null ? 'N/A' : formatValue(summaryItem),
      fulfilled,
      applicable,
  ...
  const activeScope = useMemo(() => {
    if (normalizeSearch(deferredStoreQuery)) return `Búsqueda · ${visibleStores.length} ${visibleStores.length === 1 ? 'tienda' : 'tiendas'}`
    if (dm !== 'Todos') return `Distrito · ${dm}`
    if (region !== 'Todas') return `Región · ${region}`
    if (storeType !== 'Todos') return `Tipo · ${storeType.replace('_',' ')}`
    return 'Portafolio completo'
  }, [deferredStoreQuery, visibleStores.length, dm, region, storeType])
  ...
  return <div className={presentationMode ? 'presentation-mode' : undefined}>
    ...
    <section className="indicator-feed card mb-5" aria-labelledby="indicator-feed-title">
      <div className="indicator-feed-heading">
        <div><p className="eyebrow">Lectura activa</p><h2 id="indicator-feed-title" className="section-title">Pulso de la selección</h2></div>
        <div className="active-scope" aria-live="polite"><span>{activeScope}</span><strong>{title}</strong></div>
      </div>
      <div className="indicator-story-rail" role="list" aria-label={`Resumen de indicadores para ${activeScope}`}>
        {indicatorSummaries.map(summary => <article key={summary.indicator} className={`indicator-story pillar-${summary.pillar.toLowerCase()}`} role="listitem">
          <div className="indicator-story-top"><span>{summary.pillar}</span><strong>{visibleIndicatorName(summary.indicator)}</strong></div>
          <p className="indicator-story-value">{summary.value}</p>
          <div className="indicator-story-foot"><span>{summary.fulfilled}/{summary.applicable} cumplen</span><strong>{summary.applicable ? `${(summary.compliance * 100).toFixed(0)}%` : 'N/A'}</strong></div>
        </article>)}
      </div>
    </section>

    <section id="ranking-regional" className="card overflow-hidden p-0">
